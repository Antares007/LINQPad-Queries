...
void Main()
{
	var dic = PolisisChabarebisIstoriebi.AsParallel()
			 	 .GroupBy (pci => pci.PaketisNomeri)
				 .ToDictionary (k => k.Key, v => v.ToList());

	var vizitebi = PolisisChabarebisIstoriebi
					.GroupBy(pci => new {
								pci.PolisisNomeri, 
								pci.Chambarebeli, 
								pci.Polisi.MzgveveliKompaniisKodi,
								pci.Polisi.ShekmnisTarigi})
					.Select(g =>
						new {
								g.Key.PolisisNomeri,
								g.Key.Chambarebeli,
								g.Key.MzgveveliKompaniisKodi,
								g.Key.ShekmnisTarigi,
								Chabarda=g.Any (x => x.Statusi=="Chabarda" ) ? "Chabarda" : "VerChabarda",
								KontraktiGaformda=g.Any (x => x.Polisi.PolisisStatusi=="Aktiuri" || x.Polisi.PolisisStatusi=="KontraqtisMoqmedebisVadaAmoicura") ? "KontraktiGaformda" : "KontraktiVerGaformda"
							})
					.ToList().AsParallel()
					.Select (x => new { 
								x.PolisisNomeri,
								x.Chabarda,
								x.MzgveveliKompaniisKodi,
								x.KontraktiGaformda,
								x.Chambarebeli,
	...
	Ext.ToExcel(vizitebi, "GankhorcielebuliVizitebi3");
