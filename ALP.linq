...
{
Ext.ToExcel (
	Polisebi.Where (p => p.ShekmnisTarigi.Month==11 && p.MzgveveliKompaniisKodi=="ALP")
		...
		p => new 
			{
				...
				p.UnikaluriKodi,
				p.PiradiNomeri,
				p.SadazgveoperiodisDasackisi,
				p.MovlenebiGaformdaGaukmdaKontrakti.Where(y=>y.Dro==p.MovlenebiGaformdaGaukmdaKontrakti.Max(x=>x.Dro)).First ().Statusi
			}), "PolisebiDekemberiALP");
}
