...
void Main()
{
	var q= PaketisChabarebisIstoriebi
		.Where (x => x.Chambarebeli=="Socagenti" && x.Chabarda)
		.Where (x => x.PolisisChabarebisIstoriebi.Any (pci => pci.Polisi.PolisisStatusi == "Aktiuri"))
		
		.GroupBy (x => new{x.ChambareblisKodi,x.Raioni,x.PolisisChabarebisIstoriebi.First ().Polisi.ShekmnisTarigi.Month})
		.Select (g => new {g.Key.ChambareblisKodi,g.Key.Raioni,g.Key.Month,Raodenoba=g.Count()}).ToList();
	Ext.ToExcel(q,"Reporti");	
}
