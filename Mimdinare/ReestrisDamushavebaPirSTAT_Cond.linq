...
").ToList();
	...
	.OrderBy(x=>x.tarigi)
	.Select(x=>@"
INSERT INTO PirStatCvlilebeli(PID,CONDITION_ID,STATUS_ID,Tarigi)
SELECT PID,CONDITION_ID,STATUS_ID,'"+x.tarigi+@"' FROM (
						SELECT PID collate Latin1_General_BIN PID,CONDITION_ID,STATUS_ID FROM "+x.DB+@".dbo."+x.TNAME+@" WHERE ISNUMERIC(PID)=1
						EXCEPT 
						SELECT PID collate Latin1_General_BIN,CONDITION_ID,STATUS_ID
						FROM PirStatCvlilebeli yvela
						join (SELECT Max(Id) Id FROM PirStatCvlilebeli GROUP BY PID) bolo on yvela.Id=bolo.Id
						) R
GO
")
